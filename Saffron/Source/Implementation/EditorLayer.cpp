...
void EditorLayer::OnProjectChange(const Shared <Project> &project) {
    ...
    const auto sceneFullFilepath = m_Project->GetProjectFolderpath().string() + m_Project->GetSceneFilepaths().front().string();
    LoadProjectScene(sceneFullFilepath);
}
        ...
        if (ImGui::BeginMenu("Script")) {
            ...
            if (ImGui::MenuItem("Reload C# Assembly") && m_Project->IsValid()) {
                ScriptEngine::ReloadAssembly(
                        "Resources/Assets/Scripts/" + m_Project->GetProjectFilepath().stem().string() + ".dll");
            }
            if (m_SceneState == SceneState::Play) {
                ImGui::PopItemFlag();
                ImGui::PopStyleVar();
