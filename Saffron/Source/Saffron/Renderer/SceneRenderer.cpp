...
void SceneRenderer::Init()
{
	...
	s_Data.CompositeShader = Shader::Create(Filepath{ "Resources/Assets/Shaders/SceneComposite.glsl" });
	const auto outlineShader = Shader::Create(Filepath{ "Resources/Assets/Shaders/Outline.glsl" });
	s_Data.OutlineMaterial = MaterialInstance::Create(Material::Create(outlineShader));
	s_Data.BRDFLUT = Texture2D::Create("Resources/Assets/Textures/BRDF_LUT.tga");

	// Grid
	const auto gridShader = Shader::Create(Filepath{ "Resources/Assets/Shaders/Grid.glsl" });
	...
	// Lines
	{
		s_Data.LineShader = Shader::Create(Filepath{ "Resources/Assets/Shaders/Renderer2D_Line.glsl" });
...
Pair<Shared<TextureCube>, Shared<TextureCube>> SceneRenderer::CreateEnvironmentMap(const String &filepath)
{
	...
	if ( !equirectangularConversionShader )
		equirectangularConversionShader = Shader::Create(Filepath{ "Resources/Assets/Shaders/EquirectangularToCubeMap.glsl" });
	...
	if ( !envFilteringShader )
		envFilteringShader = Shader::Create(Filepath{ "Resources/Assets/Shaders/EnvironmentMipFilter.glsl" });
	...
	if ( !envIrradianceShader )
		envIrradianceShader = Shader::Create(Filepath{ "Resources/Assets/Shaders/EnvironmentIrradiance.glsl" });
