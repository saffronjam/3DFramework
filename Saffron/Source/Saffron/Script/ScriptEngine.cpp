...
{
	mono_set_assemblies_path("Dependencies/mono/bin/Runtime");
...
void ScriptEngine::OnProjectChange(const std::shared_ptr<Project> &project)
{
	...
	if ( project )
	{
		LoadRuntimeAssembly("Resources/Assets/Scripts/" + s_ProjectContext->GetName() + ".dll");
	}
}
...
void ScriptEngine::LoadRuntimeAssembly(const Filepath &assemblyFilepath)
{
	...
	s_CoreAssembly = LoadAssembly("Resources/Assets/Scripts/ScriptCore.dll");
