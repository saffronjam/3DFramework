...
{
	m_Initializer = [this]() mutable
	{
		const auto instance = OpenGLTexture2D::GetDynShared<OpenGLTexture2D>();
		...
		m_ImageData.Allocate(m_Width * m_Height * GetBPP(m_Format));
	};
}

OpenGLTexture2D::OpenGLTexture2D(Filepath filepath, bool sRGB)
	: m_Format(Format::None), m_Width(0), m_Height(0), m_Filepath(Move(filepath))
{
	m_Initializer = [this, sRGB]() mutable
	{
		...
		auto instance = OpenGLTexture2D::GetDynShared<OpenGLTexture2D>();
...
{
	auto instance = GetDynShared<OpenGLTexture2D>();
	...
	: m_RendererID(0), m_Format(format), m_Width(width), m_Height(height)
{
	...
	m_Initializer = [this, levels]() mutable
	{
		auto instance = OpenGLTextureCube::GetDynShared<OpenGLTextureCube>();
	...
	: m_Format(Format::None), m_Width(0), m_Height(0), m_Filepath(Move(filepath))
{
	m_Initializer = [this]()
	{
		...
		for ( auto &face : faces )
		{
			face = new Uint8[static_cast<Uint32>(faceWidth * faceHeight) * 3]; // 3 BPP
		}
...
{
	auto instance = GetDynShared<OpenGLTextureCube>();
	Renderer::Submit([instance, slot]() { glBindTextureUnit(slot, instance->m_RendererID); });
}
