...
void SceneRenderer::Init()
{
	...
	s_Data.CompositeShader = Shader::Create(Filepath{ "res/Assets/Shaders/SceneComposite.glsl" });
	const auto outlineShader = Shader::Create(Filepath{ "res/Assets/Shaders/Outline.glsl" });
	s_Data.OutlineMaterial = MaterialInstance::Create(Material::Create(outlineShader));
	s_Data.BRDFLUT = Texture2D::Create("res/Assets/Textures/BRDF_LUT.tga");

	// Grid
	const auto gridShader = Shader::Create(Filepath{ "res/Assets/Shaders/Grid.glsl" });
...
Pair<Shared<TextureCube>, Shared<TextureCube>> SceneRenderer::CreateEnvironmentMap(const String &filepath)
{
	...
	if ( !equirectangularConversionShader )
		equirectangularConversionShader = Shader::Create(Filepath{ "res/Assets/Shaders/EquirectangularToCubeMap.glsl" });
	...
	if ( !envFilteringShader )
		envFilteringShader = Shader::Create(Filepath{ "res/Assets/Shaders/EnvironmentMipFilter.glsl" });
	...
	if ( !envIrradianceShader )
		envIrradianceShader = Shader::Create(Filepath{ "res/Assets/Shaders/EnvironmentIrradiance.glsl" });
