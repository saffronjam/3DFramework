...
void EditorLayer::OnAttach(Shared <BatchLoader> &loader) {
    ...
    loader->Submit([this] {
        m_TexStore["Checkerboard"] = Texture2D::Create("res/Assets/Editor/Checkerboard.tga");
        m_TexStore["Checkerboard2"] = Texture2D::Create("res/Assets/Editor/Checkerboard.tga");
        m_TexStore["PlayButton"] = Texture2D::Create("res/Assets/Editor/PlayButton.png");
        m_TexStore["PauseButton"] = Texture2D::Create("res/Assets/Editor/PauseButton.png");
        m_TexStore["StopButton"] = Texture2D::Create("res/Assets/Editor/StopButton.png");
        m_TexStore["TranslateButton"] = Texture2D::Create("res/Assets/Editor/Translate_w.png");
        m_TexStore["RotateButton"] = Texture2D::Create("res/Assets/Editor/Rotate_w.png");
        m_TexStore["ScaleButton"] = Texture2D::Create("res/Assets/Editor/Scale_w.png");
        m_TexStore["ControllerGameButton"] = Texture2D::Create("res/Assets/Editor/ControllerGame_w.png");
        m_TexStore["ControllerMayaButton"] = Texture2D::Create("res/Assets/Editor/ControllerMaya_w.png");
    ...
    loader->Submit([this] {
        const auto &postRenderFn = [this]() {
            ...
            if (m_SceneState == SceneState::Edit &&
                ...
                if (m_SelectionMode == SelectionMode::Entity) {
                    ...
                    glm::mat4 transformBase =
                            entityTransform * modelEntity.GetComponent<TransformComponent>().Transform;
                    ImGuizmo::Manipulate(glm::value_ptr(editorCamera->GetViewMatrix()),
                                         glm::value_ptr(editorCamera->GetProjectionMatrix()),
                                         static_cast<ImGuizmo::OPERATION>(m_GizmoType),
                                         ImGuizmo::WORLD,
                    ...
                    modelEntity.GetComponent<TransformComponent>().Transform =
                            glm::inverse(entityTransform) * transformBase;
                }
            }
        };
        ...
        m_ScenePanel->GetSignal(ScenePanel::Signals::OnEntityDeleted).Connect(
                [this](Entity entity) { OnEntityDeleted(entity); });
        m_ScenePanel->GetSignal(ScenePanel::Signals::OnEntityCopied).Connect(
                [this](Entity entity) { OnEntityCopied(entity); });
        m_ScenePanel->GetSignal(ScenePanel::Signals::OnViewInModelSpace).Connect(
                [this](Entity entity) { OnNewModelSpaceView(entity); });
        ...
        case SceneState::Edit: {
            ...
            if (m_SelectedEntity) {
                if (m_SelectedEntity.HasComponent<BoxCollider2DComponent>()) {
                    ...
                    auto[translation, rotationQuat, scale] = Misc::GetTransformDecomposition(
                            m_SelectedEntity.GetComponent<TransformComponent>().Transform);
        ...
        if (MousePosition.x > -1.0f && MousePosition.x < 1.0f && MousePosition.y > -1.0f && MousePosition.y < 1.0f) {
            ...
            auto raycast = [&selections, &rayResult](Entity entity, const Shared <Mesh> &mesh) {
                ...
                for (Uint32 i = 0; i < submeshes.size(); i++) {
                    ...
                    Ray ray = {
                            glm::inverse(entity.Transform() * mesh->GetLocalTransform() * submesh.Transform) *
                            Vector4f(rayResult.first, 1.0f),
                            glm::inverse(Matrix3f(entity.Transform()) * Matrix3f(submesh.Transform) *
                                         Matrix3f(mesh->GetLocalTransform())) * rayResult.second
                    };
...
void EditorLayer::PromptNewScene() {
    ...
    if (!filepath.empty()) {
        ...
        newScene->SetEnvironment(Scene::Environment::Load("res/Assets/Env/birchwood_4k.hdr"));
...
void EditorLayer::OnProjectChange(const Shared <Project> &project) {
    ...
    m_Project = project;
    m_AssetPanel->SetAssetFolderpath(m_Project->GetProjectFolderpath().string() + "res/Assets/Meshes");
...
void EditorLayer::OnPlay() {
    ...
    if (m_ReloadScriptOnPlay && m_Project->IsValid()) {
        ScriptEngine::ReloadAssembly("res/Assets/Scripts/" + m_Project->GetProjectFilepath().stem().string() + ".dll");
    }
        ...
        if (ImGui::BeginMenu("Script")) {
            ...
            if (ImGui::MenuItem("Reload C# Assembly") && m_Project->IsValid()) {
                ScriptEngine::ReloadAssembly(
                        "res/Assets/Scripts/" + m_Project->GetProjectFilepath().stem().string() + ".dll");
            }
            if (m_SceneState == SceneState::Play) {
                ImGui::PopItemFlag();
                ImGui::PopStyleVar();
...
void EditorLayer::OnGuiRenderToolbar() {
    ...
    Shared <Texture> ButtonTexture =
            m_SceneState == SceneState::Edit ? m_TexStore["PlayButton"] : m_TexStore["StopButton"];
    ...
    if (scene && scene->GetEntity().HasComponent<EditorCameraComponent>()) {
        const auto &editorCamera = scene->GetEntity().GetComponent<EditorCameraComponent>().Camera;
        ControllerWASDTint =
                editorCamera->GetControllerStyle() == EditorCamera::ControllerStyle::Game ? ImGuiBlue : ClearWhite;
        ControllerMayaTint =
                editorCamera->GetControllerStyle() == EditorCamera::ControllerStyle::Maya ? ImGuiBlue : ClearWhite;
    } else {
