...
{
	mono_set_assemblies_path("deps/mono/bin/Runtime");
...
void ScriptEngine::OnProjectChange(const Shared<Project> &project)
{
	...
	if ( project )
	{
		LoadRuntimeAssembly("res/Assets/Scripts/" + s_ProjectContext->GetName() + ".dll");
	}
}
...
void ScriptEngine::LoadRuntimeAssembly(const Filepath &assemblyFilepath)
{
	...
	s_CoreAssembly = LoadAssembly("res/Assets/Scripts/ScriptCore.dll");
