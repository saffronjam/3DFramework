	...
	TestObject(gfx, rng, adist, ddist, odist, rdist)
{
	...
	if ( !IsInitialized() )
	{
		...
		Assimp::Importer imp;
		const auto pModel = imp.ReadFile("res/models/suzanne.obj",
		...
		pmc.color = material;
		AddStaticBind(std::make_unique<PSConstantBuffer<PSMaterialConstant>>(gfx, pmc, 1u));
	}
	else
	{
		SetIndexFromStatic();
