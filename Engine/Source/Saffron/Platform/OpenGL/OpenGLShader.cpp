...
OpenGLShader::OpenGLShader(Filepath filepath) :
...
OpenGLShader::OpenGLShader(const Buffer& source)
{
	Load(source);
}
...
Buffer OpenGLShader::ReadFromFile(const Filepath& filepath) const
{
	String result;
...
UnorderedMap<GLenum, String> OpenGLShader::PreProcess(const Buffer& source)
{
	...
	const String view(reinterpret_cast<const char*>(source.Data()), source.Size() / sizeof(String::value_type));

	const char* typeToken = "#type";
	const size_t typeTokenLength = strlen(typeToken);
	size_t pos = view.find(typeToken, 0);
	while (pos != String::npos)
	{
		const size_t eol = view.find_first_of("\r\n", pos);
		SE_CORE_ASSERT(eol != String::npos, "Syntax error");
		const size_t begin = pos + typeTokenLength + 1;
		auto type = view.substr(begin, eol - begin);
		SE_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel" || type == "compute",
		               "Invalid shader type specified");

		const size_t nextLinePos = view.find_first_not_of("\r\n", eol);
		pos = view.find(typeToken, nextLinePos);
		auto shaderType = ShaderTypeFromString(String(type));
		shaderSources[shaderType] = view.substr(nextLinePos,
		                                        pos - (nextLinePos == String::npos ? view.size() - 1 : nextLinePos));
	...
	else
	{
		...
		if (StartsWith(name, "r_"))
		{
			if (domain == ShaderDomain::Vertex)
				dynamic_cast<OpenGLShaderUniformBufferDeclaration*>(m_VSRendererUniformBuffers.front())->PushUniform(
					declaration);
			else if (domain == ShaderDomain::Pixel)
				dynamic_cast<OpenGLShaderUniformBufferDeclaration*>(m_PSRendererUniformBuffers.front())->PushUniform(
					declaration);
		}
		else
		{
...
void OpenGLShader::ResolveAndSetUniforms(const Shared<OpenGLShaderUniformBufferDeclaration>& decl, const Buffer& buffer)
{
	...
	{
		auto* glUniform = dynamic_cast<OpenGLShaderUniformDeclaration*>(uniform);
		if (glUniform->IsArray()) ResolveAndSetUniformArray(glUniform, buffer);
		else ResolveAndSetUniform(glUniform, buffer);
	}
}
...
void OpenGLShader::ResolveAndSetUniform(OpenGLShaderUniformDeclaration* uniform, const Buffer& buffer)
{
	...
	case OpenGLShaderUniformDeclaration::Type::Bool: UploadUniformFloat(
			uniform->GetLocation(), *reinterpret_cast<const bool*>(&buffer.Data()[offset]));
		break;
	case OpenGLShaderUniformDeclaration::Type::Float32: UploadUniformFloat(
			uniform->GetLocation(), *reinterpret_cast<const float*>(&buffer.Data()[offset]));
		break;
	...
	case OpenGLShaderUniformDeclaration::Type::Vec2: UploadUniformFloat2(
			uniform->GetLocation(), *reinterpret_cast<const Vector2f*>(&buffer.Data()[offset]));
		break;
	case OpenGLShaderUniformDeclaration::Type::Vec3: UploadUniformFloat3(
			uniform->GetLocation(), *reinterpret_cast<const Vector3f*>(&buffer.Data()[offset]));
		break;
	case OpenGLShaderUniformDeclaration::Type::Vec4: UploadUniformFloat4(
			uniform->GetLocation(), *reinterpret_cast<const Vector4f*>(&buffer.Data()[offset]));
		break;
	case OpenGLShaderUniformDeclaration::Type::Mat3: UploadUniformMat3(uniform->GetLocation(),
	                                                                   *reinterpret_cast<const Matrix3f*>(&buffer.Data()
		                                                                   [offset]));
		break;
	case OpenGLShaderUniformDeclaration::Type::Mat4: UploadUniformMat4(uniform->GetLocation(),
	                                                                   *reinterpret_cast<const Matrix4f*>(&buffer.Data()
		                                                                   [offset]));
		break;
...
void OpenGLShader::ResolveAndSetUniformArray(OpenGLShaderUniformDeclaration* uniform, const Buffer& buffer)
{
	...
	case OpenGLShaderUniformDeclaration::Type::Mat4: UploadUniformMat4Array(
			uniform->GetLocation(), *reinterpret_cast<const Matrix4f*>(&buffer.Data()[offset]), uniform->GetCount());
		break;
	case OpenGLShaderUniformDeclaration::Type::Struct: UploadUniformStruct(uniform, buffer.Data(), offset);
		break;
	default: SE_CORE_ASSERT(false, "Unknown uniform type!");
...
void OpenGLShader::ResolveAndSetUniformField(const OpenGLShaderUniformDeclaration& field, const Uint8* data,
                                             Int32 offset)
{
	switch (field.GetType())
	{
	case OpenGLShaderUniformDeclaration::Type::Bool: UploadUniformFloat(
			field.GetLocation(), *reinterpret_cast<const bool*>(&data[offset]));
		break;
	case OpenGLShaderUniformDeclaration::Type::Float32: UploadUniformFloat(
			field.GetLocation(), *reinterpret_cast<const float*>(&data[offset]));
		break;
	case OpenGLShaderUniformDeclaration::Type::Int32: UploadUniformInt(field.GetLocation(),
	                                                                   *reinterpret_cast<const int*>(&data[offset]));
		break;
	case OpenGLShaderUniformDeclaration::Type::Vec2: UploadUniformFloat2(
			field.GetLocation(), *reinterpret_cast<const Vector2f*>(&data[offset]));
		break;
	case OpenGLShaderUniformDeclaration::Type::Vec3: UploadUniformFloat3(
			field.GetLocation(), *reinterpret_cast<const Vector3f*>(&data[offset]));
		break;
	case OpenGLShaderUniformDeclaration::Type::Vec4: UploadUniformFloat4(
			field.GetLocation(), *reinterpret_cast<const Vector4f*>(&data[offset]));
		break;
	case OpenGLShaderUniformDeclaration::Type::Mat3: UploadUniformMat3(field.GetLocation(),
	                                                                   *reinterpret_cast<const Matrix3f*>(&data[offset]
	                                                                   ));
		break;
	case OpenGLShaderUniformDeclaration::Type::Mat4: UploadUniformMat4(field.GetLocation(),
	                                                                   *reinterpret_cast<const Matrix4f*>(&data[offset]
	                                                                   ));
		break;
	default: SE_CORE_ASSERT(false, "Unknown uniform type!");
	}
...
const ShaderUniformBufferDeclaration::List& OpenGLShader::GetVSRendererUniforms() const
{
	return m_VSRendererUniformBuffers;
}

const ShaderUniformBufferDeclaration::List& OpenGLShader::GetPSRendererUniforms() const
{
