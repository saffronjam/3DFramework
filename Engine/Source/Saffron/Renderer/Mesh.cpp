	...
	m_Filepath(MeshesFolder + Move(filepath).string())
{
	...
	m_MeshShader = Shared<Shader>(Factory::Create<Shader>(meshShaderPath));
	...
	if (m_IsAnimated)
	{
		m_VertexBuffer = Factory::Create<VertexBuffer>(m_AnimatedVertices.data(),
	...
	{
		m_VertexBuffer = Factory::Create<VertexBuffer>(m_StaticVertices.data(),
	...
	m_IndexBuffer = Factory::Create<IndexBuffer>(m_Indices.data(),
