	...
	Instansiated(this)
{
	...
	Factory::Create<Shader>("SaffronPBR_Static");
	Factory::Create<Shader>("SaffronPBR_Anim");
	...
	// Create fullscreen quad
	{
		...
		Specification.Layout = {{ShaderDataType::Float3, "a_Position"}, {ShaderDataType::Float2, "a_TexCoord"}};
		_data.m_FullscreenQuadPipeline = Factory::Create<Pipeline>(Specification);

		_data.m_FullscreenQuadVertexBuffer = Factory::Create<VertexBuffer>(data, 4 * sizeof(QuadVertex));
		Uint32 indices[6] = {0, 1, 2, 2, 3, 0,};
		_data.m_FullscreenQuadIndexBuffer = Factory::Create<IndexBuffer>(indices, 6 * sizeof(Uint32));
	}

	// Lines
	{
		Pipeline::Specification Specification;
		Specification.Layout = {{ShaderDataType::Float3, "a_Position"}, {ShaderDataType::Float4, "a_Color"}};
		_data.LinePipeline = Factory::Create<Pipeline>(Specification);

		_data.LineVertexBuffer = Factory::Create<VertexBuffer>(RendererData::MaxLineVertices * sizeof(LineVertex));
		auto* lineIndices = new Uint32[RendererData::MaxLineIndices];
		for (Uint32 i = 0; i < RendererData::MaxLineIndices; i++) lineIndices[i] = i;
		_data.LineIndexBuffer = Factory::Create<IndexBuffer>(lineIndices, RendererData::MaxLineIndices);
		delete[] lineIndices;
	}
}
...
{
	_data.m_CommandQueue.Submit(func);
}

void Renderer::Execute()
{
	_data.m_CommandQueue.Execute();
}
...
void Renderer::BeginRenderPass(Shared<RenderPass> renderPass, bool clear)
{
	...
	_data.m_ActiveRenderPass = renderPass;
...
{
	SE_CORE_ASSERT(_data.m_ActiveRenderPass, "No active render pass! Have you called Renderer::EndRenderPass twice?");
	_data.m_ActiveRenderPass->GetSpecification().TargetFramebuffer->Unbind();
	_data.m_ActiveRenderPass = nullptr;
}
...
void Renderer::SubmitQuad(Shared<MaterialInstance> material, const Matrix4f& transform)
{
	...
	_data.m_FullscreenQuadVertexBuffer->Bind();
	_data.m_FullscreenQuadPipeline->Bind();
	_data.m_FullscreenQuadIndexBuffer->Bind();
...
void Renderer::SubmitLines(LineVertex* vertices, Uint32 count, Shared<Shader> shader)
{
	...
	if (dataSize)
	{
		...
		_data.LineVertexBuffer->SetData(vertices, dataSize);

		_data.LineVertexBuffer->Bind();
		_data.LinePipeline->Bind();
		_data.LineIndexBuffer->Bind();
	...
	for (Submesh& submesh : mesh->m_Submeshes)
	{
		...
		Submit([submesh, material]()
		{
			...
			glDrawElementsBaseVertex(GL_TRIANGLES, submesh.IndexCount, GL_UNSIGNED_INT,
			                         reinterpret_cast<void*>(sizeof(Uint32) * submesh.BaseIndex), submesh.BaseVertex);
		});
	}
}

void Renderer::SubmitMesh(Shared<Mesh> mesh, const Matrix4f& transform, Shared<Shader> shader)
{
	mesh->m_VertexBuffer->Bind();
	mesh->m_Pipeline->Bind();
	mesh->m_IndexBuffer->Bind();
