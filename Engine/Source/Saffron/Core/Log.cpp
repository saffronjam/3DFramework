...
{
	auto& sinks = const_cast<ArrayList<spdlog::sink_ptr>&>(Instance()._coreLogger->sinks());
...
{
	auto& sinks = const_cast<ArrayList<spdlog::sink_ptr>&>(Instance()._clientLogger->sinks());
