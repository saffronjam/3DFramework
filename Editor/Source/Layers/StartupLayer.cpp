...
void StartupLayer::OnGuiRender()
{
	...
	const auto windowFlags = ImGuiWindowFlags_NoDocking | ImGuiWindowFlags_NoNavFocus | ImGuiWindowFlags_NoTitleBar |
	...
	if (ImGui::Begin("ProjectSelector##BackgroundPreview", nullptr, windowFlags))
	{
		...
		if (height > 0)
		{
			...
			if (ImGui::Begin("ProjectSelector##Selector", nullptr,
			                 ImGuiWindowFlags_NoDocking | ImGuiWindowFlags_NoNavFocus | ImGuiWindowFlags_NoTitleBar |
			                 ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove))
			{
				ImGui::PopStyleVar(3);
				...
				{
					return project->LastOpened().Year() == currentDate.Year() && project->LastOpened().Month() ==
						currentDate.Month();
				};
				auto LastMonthOperator = [&currentDate](const Shared<Project>& project)
				{
					return project->LastOpened().Year() == currentDate.Year() && project->LastOpened().Month() ==
						...
						project->LastOpened().Year() == currentDate.Year() - 1 && project->LastOpened().Month() == 12 &&
						currentDate.Month() == 1;
				};
				auto OlderOperator = [&currentDate, &RecentOperator, &LastMonthOperator](const Shared<Project>& project)
				{
					return !RecentOperator(project) && !LastMonthOperator(project) && project->LastOpened() <
						currentDate;
				};
				                                                   ...
				                                                   bool open)
				{
					...
					if (ImGui::TreeNodeEx(name, nodeFlags))
					{
						...
						Gui::SetFontSize(18);
						for (auto iter = firstIter; iter != projectList.end() && std::distance(firstIter, iter) < num;
						     ++iter)
						{
							PutProjectItem(*iter);
						}
		...
		ImGui::SetCursorPos({
			ImGui::GetWindowWidth() * 0.10f + buttonSize.x, 4.0f * ImGui::GetWindowHeight() / 5.0f - buttonSize.y
		});
