	...
	loader->Submit([this]
				   {
					   const auto &postRenderFn = [this]()
					   {
							   ...
							   modelEntity )
						   {
							   ...
							   {
								   auto boundingBoxes = m_SelectedEntity.GetComponent<MeshComponent>().Mesh->GetBoundingBoxes(m_SelectedEntity.GetComponent<TransformComponent>().Transform);
								   auto &first = boundingBoxes.front();
								   bounds[0] = first.Min.x; bounds[1] = first.Min.y; bounds[2] = first.Min.z;
								   bounds[3] = first.Max.x; bounds[4] = first.Max.y; bounds[5] = first.Max.z;
								   boundsPtr = bounds;
							   }
							   else
							   {
		...
		if ( MousePosition.x > -1.0f && MousePosition.x < 1.0f && MousePosition.y > -1.0f && MousePosition.y < 1.0f )
		{
			...
			auto raycast = [&selections, &rayResult](Entity entity, const Shared<Mesh> &mesh)
			{
				...
				for ( Uint32 i = 0; i < submeshes.size(); i++ )
				{
					...
					Ray ray = {
						glm::inverse(entity.Transform() * mesh->GetLocalTransform() * submesh.Transform) * Vector4f(rayResult.first, 1.0f),
						glm::inverse(Matrix3f(entity.Transform()) * Matrix3f(submesh.Transform) * Matrix3f(mesh->GetLocalTransform())) * rayResult.second
					};
...
void EditorLayer::OnPlay()
{
	...
	{
		ScriptEngine::ReloadAssembly("Assets/Scripts/" + m_Project->GetProjectFilepath().stem().string() + ".dll");
	}
...
void EditorLayer::UpdateWindowTitle(String sceneName)
{
	const String title = Move(sceneName) + " - SaffronBun - " + Application::GetPlatformName() + " (" + Application::GetConfigurationName() + ")";
...
void EditorLayer::OnGuiRenderToolbar()
{
	...
	{
		if ( m_SceneState == SceneState::Edit )
			scene->GetEntity().GetComponent<EditorCameraComponent>().Camera->SetControllerStyle(EditorCamera::ControllerStyle::Game);
	}
	ImGui::SameLine();
	...
	{
		if ( m_SceneState == SceneState::Edit )
			scene->GetEntity().GetComponent<EditorCameraComponent>().Camera->SetControllerStyle(EditorCamera::ControllerStyle::Maya);
	}
